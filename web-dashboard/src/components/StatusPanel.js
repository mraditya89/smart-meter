...
export default function StatusPanel(param) {
  ...
  const passwordValidate = async () => {
    await axios
      .get(`${apiUrlCheck}/check-operator/${user}/${password}`, {
  ...
  const relayFunction = async () => {
    axios
      ...
      .then((res) => {
        console.log(res);
        window.alert(
          "Perintah berehasil dilakukan, mohon tunggu data terudate"
        );
  ...
  return (
    <React.Fragment>
      ...
      <Button
        style={{ fontSize: "2em" }}
        color={statusMeteran === "On" ? "primary" : "secondary"}
      >
        {statusMeteran === "On" ? (
          <EmojiObjectsIcon style={{ marginRight: "0.2em" }} />
        ) : (
          <HighlightOffIcon style={{ marginRight: "0.2em" }} />
        )}
        {statusMeteran}
      </Button>
      ...
      <div>
        ...
        >
          <Button
            variant="outlined"
            color={relayText === "Nyalakan" ? "primary" : "secondary"}
