...
createUnit = async (req, res) => {
    ...
    await UnitRegis.findOne({ reg_id: body.reg_id }, (err, data) => {
        ...
        data.remove().catch(err => console.log(err))
    ...
    }).sort({unit_id: -1}).catch(err => console.log(err))

    const bd = {
        "unit_id": unit_id,
        "pin": "$2a$10$bx2pbZ0d/3hvtlC7HxT7peVoXRD8ONqxpjXTe7PYIASJpF6FPztTK",
        "alamat": body.alamat,
        "kota": body.kota,
        ...
        "is_new_key": false,
        "is_responding": false,
        "last_known_key": "U2FsdGVkX18AXiIwWznpWx86WqqxYLvu54XUQlzKk1psSUcllpFUWoI3pgr/UrFissqwuHuNNy28Ih9NaTN1sw==",
        "email": body.email
    }
...
getUnitLocation = async (req, res) => {
    await Unit.find({}, (err, unit) => {
        ...
        return res.status(200).json({ success: true, data: { label: label, count: count } })
    }).catch(err => console.log(err))
}
