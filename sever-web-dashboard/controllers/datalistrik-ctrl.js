...
getDataListrikAkhir = async (req, res) => {
    ...
    }).sort({ date_time: -1 }).limit(10).catch(err => console.log(err))
}
...
getDataListrikTotal = async (req, res) => {
    await DataListrik.find({ unit_id: req.params.id }, (err, data) => {
        ...
        return res.status(200).json({ success: true, data: dataFilter })
    }).sort({ date_time: -1 }).catch(err => console.log(err))
}
...
getCurrentYearDataListrikById = async (req, res) => {
    ...
    let dataDaya = [];
    await DataListrik.find({ unit_id: req.params.id, date_time: {$gte: currentYear, $lte: nextYear} }, (err, data) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        // if (!data.length) {
...
getCurrentYearDataListrik = async (req, res) => {
    let dt = new Date();
    let currentYear = dt.getFullYear().toString();
    let nextYear = (dt.getFullYear()+1).toString();
    let dataDaya = [];
    await DataListrik.find({ date_time: {$gte: currentYear, $lte: nextYear} }, (err, data) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        // if (!data.length) {
...
getLastYearDataListrik = async (req, res) => {
    let dt = new Date();
    let lastYear = (dt.getFullYear()-1).toString();
    let currentYear = dt.getFullYear().toString();
    let dataDaya = [];
    await DataListrik.find({ date_time: {$gte: lastYear, $lte: currentYear} }, (err, data) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        // if (!data.length) {
